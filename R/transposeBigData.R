...
transposeBigData <- function (x, blocksize = 20000) {
    ...
    if (blocksize < 2) {
        stop("The blocksize should be a positive integer > 1.")
    }
    nrow1 <- nrow(x)
    ncol1 <- ncol(x)
    ...
    if (nrow1 <= ncol1) {
        ...
        } else if (ncol1 - no.blocks * blocksize == 1) {
        ...
        } else if (nrow1 - no.blocks * blocksize == 1) {
    ...
    if (isdataframe) {
        ...
        xTranspose <- as.matrix(xTranspose)
    }
    colnames(xTranspose) <- rownames(x)
    rownames(xTranspose) <- colnames(x)
