                           ...
                           cex.labels = 1, ..., adjustRange = FALSE)
{
    ...
    for (m in 1:nMerge) {
        ...
        o2 = tree$merge[m, 2]
        h = heights[m]
        hh = ifelse(hang>0, h-hang.scaled, range[1])
        h1 = ifelse(o1 < 0, hh, heights[o1])
        h2 = ifelse(o2 < 0, hh, heights[o2])

        x1 = ifelse(o1 < 0, singleton.x[-o1], cluster.x[o1])
        x2 = ifelse(o2 < 0, singleton.x[-o2], cluster.x[o2])
        ...
        if (!is.null(labels)) {
            if (horiz) {
                if (o1 < 0) {
                    text(h1, x1, paste(labels[-o1], " "), adj = c(0, 0.5),
                         srt = 0,cex = cex.labels, xpd = TRUE)
                }
                if (o2 < 0) {
                    text(h2, x2, paste(labels[-o2], " "), adj = c(0, 0.5),
                         srt = 0, cex = cex.labels, xpd = TRUE)
                }
            } else {
                if (o1 < 0) {
                    text(x1, h1, paste(labels[-o1], " "), adj = c(1, 0.5),
                         srt = 90, cex = cex.labels, xpd = TRUE)
                }
                if (o2 < 0) {
                    text(x2, h2, paste(labels[-o2], " "), adj = c(1, 0.5),
                         srt = 90, cex = cex.labels, xpd = TRUE)
                }
            }
        }
