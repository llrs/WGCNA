...
plotCor <- function(x, new = FALSE, nrgcols = 50, labels = FALSE, labcols = 1,
    ...
    image(1:n, 1:n, corr[, n:1], col = rgcolors.func(nrgcols), axes = FALSE,
          xlab = "", ylab = "", ... )
    ...
    if (length(labcols) == n) {
        ...
        for(i in 1:length(cols)) {
            ...
            axis(2, at = (n:1)[which], labels = labels[which], las = 2,
...
#' \code{\link{cor}}, \code{\link{image}}, \code{\link{rgb}}.
#' @keywords hplot
plotMat <- function(x, nrgcols = 50, rlabels = FALSE, clabels = FALSE,
                    rcols = 1, ccols = 1, title = "", ...) {
    #  X  <- x
    n <- nrow(x)
    p <- ncol(x)

    image(1:p, 1:n, t(x[n:1, ]), col = rgcolors.func(nrgcols), axes = FALSE,
          xlab = "", ylab = "", ... )
    ...
    if (length(rcols) == n) {
        ...
        for(i in 1:length(cols)) {
            ...
            axis(2, at = (n:1)[which], labels = rlabels[which], las = 2,
                 cex.axis = 0.6, col.axis = cols[i])
        }
    }
