...
#' following R packages \code{entropy}, \code{minet}, \code{infotheo} since the
#' function \code{mutualInfoAdjacency} makes use of the \code{entropy} function
#' from the R package \code{entropy} (Hausser and Strimmer 2008) and functions
#' from the \code{minet} and \code{infotheo} package (Meyer et al 2008).  A
#' weighted network adjacency matrix is a symmetric matrix whose entries take
#' on values between 0 and 1. Each weighted adjacency matrix contains scaled
#' versions of the mutual information between the columns of the input data
#' frame \code{datE}. We assume that datE contains numeric values which will be
#' discretized unless the user chooses the option
#' \code{discretizeColumns=FALSE}. The raw (unscaled) mutual information and
#' entropy measures have units "nat", i.e. natural logarithms are used in their
#' definition (base e=2.71..).  Several mutual information estimation methods
#' have been proposed in the literature (reviewed in Hausser and Strimmer 2008,
#' Meyer et al 2008).  While mutual information networks allows one to detect
#' non-linear relationships between the columns of \code{datE}, they may
...
#' The number of nodes in the mutual information network equals the number of
#' columns of \code{datE}.
#' @param discretizeColumns is a logical variable. If it is set to TRUE then
#' the columns of \code{datE} will be discretized into a user-defined number of
#' bins (see \code{numberBins}).
#' @param entropyEstimationMethod takes a text string for specifying the
#' entropy and mutual information estimation method. If
#' \code{entropyEstimationMethod="MM"} then the Miller-Madow asymptotic bias
#' corrected empirical estimator is used.  If
#' \code{entropyEstimationMethod="ML"} the maximum likelihood estimator (also
#' known as plug-in or empirical estimator) is used. If
#' \code{entropyEstimationMethod="shrink"}, the shrinkage estimator of a
#' Dirichlet probability distribution is used. If
#' \code{entropyEstimationMethod="SG"}, the Schurmann-Grassberger estimator of
#' the entropy of a Dirichlet probability distribution is used.
#' @param numberBins is an integer larger than 0 which specifies how many bins
#' are used for the discretization step. This argument is only relevant if
#' \code{discretizeColumns} has been set to TRUE. By default \code{numberBins}
...
#' (http://en.wikipedia.org/wiki/Mutual_information), this vector contains the
#' values Hx where x corresponds to a column in \code{datE}.  }
#' \item{MutualInformation}{ is a symmetric matrix whose entries contain the
#' pairwise mutual information measures between the columns of \code{datE}. The
#' diagonal of the matrix \code{MutualInformation} equals \code{Entropy}. In
...
#' \code{AdjacencySymmetricUncertainty}=2*I(X;Y)/(H(X)+H(Y)). Since
...
#' AdjacencySymmetricUncertainty)}. Note that f(x)= x/(2-x) is a monotonically
#' increasing function on the unit interval [0,1] whose values lie between 0
#' and 1. The reason why we call it the universal adjacency is that
#' dissUA=1-\code{AdjacencyUniversalVersion1} turns out to be a universal
...
mutualInfoAdjacency <- function(datE, discretizeColumns = TRUE,
    ...
    if (!is.element(discretizeColumns, c(TRUE, FALSE))) {
        stop("The input parameter discretizeColumns contains a value that is ",
             "not logical. It needs to be set to TRUE or FALSE")
    }
    ...
    if (!(numberBins>1)) {
        stop("Something is wrong with the input parameter numberBins, which is",
             " used for discretizing the quantitative variables. numberBins ",
             "should be larger than 1. Recommendation: choose the default ",
             "value numberBins=NULL")
    }
