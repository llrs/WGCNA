...
multiData.eigengeneSignificance <- function(multiData, multiTrait, moduleLabels,
    ...
    if (!is.null(useModules)) {
        keep <- modLevels %in% useModules
        if (sum(keep) == 0) {
            stop("Incorrectly specified 'useModules': no such module(s).")
        }
        if (any(!(useModules %in% modLevels))) {
            stop("Some entries in 'useModules' do not exist in the module ",
                 "labels or eigengenes.")
        }
        for (set in 1:nSets) {
            multiEigengenes[[set]]$data <- multiEigengenes[[
                set]]$data[,
                           keep,
                           drop = FALSE]
        }
    ...
    for (set in 1:nSets) {
        ...
        if (!is.null(cp$nObs)) {
            ...
            trans <- t(is.na(multiEigengenes[[set]]$data))
            nObs[, set] <- trans %*% (!is.na(multiTrait[[set]]$data))
        }
    }
    ...
    rown <- colnames(multiEigengenes[[1]]$data)
    colnames(MES) <- colnames(p) <- colnames(Z) <- colnames(nObs) <- setNames
    rownames(MES) <- rownames(p) <- rownames(Z) <- rownames(nObs) <- rown
    ...
    list(eigengeneSignificance = MES, p.value = p, q.value = q, Z = Z,
         nObservations = nObs)
}
