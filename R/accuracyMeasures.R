...
#' @references http://en.wikipedia.org/wiki/Sensitivity_and_specificity
#' @keywords misc
#' @examples
#'
#' m <- 100
...
accuracyMeasures = function(predicted, observed = NULL,
  ...
  if (type == "auto") {
    if (!is.null(dim(predicted))) {
      ...
      if (is.null(observed)) {
        stop("When 'predicted' is a vector, 'observed' must be given and have",
             " the same length as 'predicted'.")
      }
      if (length(levels) == 2) {
        type = "binary"
      } else
  ...
  if (type == "binary") {
    if (is.null(dim(predicted))) {
      ...
      if ( length(predicted) != length(observed)) {
        stop("When both 'predicted' and 'observed' are given, they must be ",
             "vectors of the same length.")
      }
      if (length(levels) != 2) {
        stop("'levels' must contain 2 entries (the possible values of the ",
      ...
      if (is.null(colnames(tab)) | is.null(rownames(tab))) {
        stop("When 'predicted' is a contingency table, it must have valid ",
    ...
    if (sum(!is.wholenumber(tab), na.rm=T) > 0){
      stop("The input table contains non-integers, which does not make sense.")
    }
    ...
    if (denom1 == 0) {
       warning("The input table has zero observations (sum of all cells is ",
               "zero).")
    }
    TP=tab[pos, pos]
    FP=tab[pos, neg]
    ...
    Power = Sensitivity
    LikelihoodRatioPositive = ifelse(1 - Specificity == 0, NA, Sensitivity / (
        1 - Specificity) )
