...
#' constructed between the columns (interpreted as nodes) of the input
#' \code{datExpr}.  If the option \code{networkType="unsigned"} then the
#' adjacency between nodes i and j is defined as
#' \code{A[i,j]=abs(cor(datExpr[,i],datExpr[,j]))^power}. In the following, we
...
#' correlation network is given by \code{A=abs(cor(datExpr,use="p"))^power} and
...
#' adjacency matrix \code{A}, the conformity vector \code{CF} is calculated by
#' requiring that \code{A[i,j]} is approximately equal to \code{CF[i]*CF[j]}.
#' Using the conformity one can define the matrix \code{A.CF=CF*t(CF)} which is
#' the outer product of the conformity vector with itself. In general,
#' \code{A.CF} is not an adjacency matrix since its diagonal elements are
#' different from 1. If the off-diagonal elements of \code{A.CF} are similar to
#' those of \code{A} according to the Frobenius matrix norm, then \code{A} is
...
#' corresponding eigengene-based gene significance measure \code{GSE}.  These
#' network concepts can only be defined for correlation networks. Details: The
#' columns (nodes) of \code{datExpr} can be summarized with the first principal
#' component, which is referred to as Eigengene in coexpression network
#' analysis. In general correlation networks, it is called eigennode. The
#' eigengene-based conformity \code{ConformityE[i]} is defined as
#' \code{abs(cor(datE[,i], Eigengene))^power} where the power corresponds to
#' the power used for defining the weighted adjacency matrix \code{A}. The
#' eigengene-based conformity can also be used to define an eigengene-based
#' adjacency matrix \code{A.E=ConformityE*t(ConformityE)}.  The eigengene based
#' factorizability \code{EF(datE)} is a number between 0 and 1 that measures
#' how well \code{A.E} approximates \code{A} when the power parameter equals 1.
#' \code{EF(datE)} is defined with respect to the singular values of
#' \code{datExpr}. For a trait based node significance measure
#' \code{GS=abs(cor(datE,trait))^power}, one can also define an eigengene-based
#' node significance measure \code{GSE[i]=ConformityE[i]*EigengeneSignificance}
#' where the eigengene significance \code{abs(cor(Eigengene,trait))^power} is
#' defined as power of the absolute value of the correlation between eigengene
#' and trait. Eigengene-based network concepts are very useful for providing a
#' geometric interpretation of network concepts and for deriving relationships
#' between network concepts. For example, the hub gene significance measure and
...
#' equal the number of samples in \code{datExpr}.
#' @param networkType network type. Recognized values are (unique abbreviations
#' of) \code{"unsigned"}, \code{"signed"}, and \code{"signed hybrid"}.
#' @return A list with the following components: \item{Summary}{a data frame
#' whose rows report network concepts that only depend on the adjacency matrix.
#' Density (mean adjacency), Centralization , Heterogeneity (coefficient of
#' variation of the connectivity), Mean ClusterCoef, Mean Connectivity. The
...
#' concept is always 1 for nodes of an unweighted network.  This is a useful
#' measure for weighted networks since it allows one to determine whether a
...
#' network concept (fundamental versus eigengene based). The first row of the
#' data frame reports the network significance. The fundamental version of this
#' network concepts is the average gene significance=mean(GS). The eigengene
#' based analog of this concept is defined as mean(GSE). The second row reports
#' the hub gene significance which is defined as slope of the intercept only
#' regression model that regresses the gene significance on the scaled network
#' connectivity K. The third row reports the eigengene significance
...
#' conformity based network concepts. These network concepts are defined for
#' any symmetric adjacency matrix (weighted and unweighted). The network
#' concepts are described in Dong and Horvath (2007) and Horvath and Dong
...
#' algorithm that maximizes the factorizability measure.  Type III: approximate
#' conformity based network concepts are functions of all elements of the
#' conformity based adjacency matrix A.CF (including the diagonal) and/or the
...
#' node), \code{Connectivity} (connectivity of each node), \code{ClusterCoef}
#' (the clustering coefficient of each node), \code{MAR} (maximum adjacency
#' ratio of each node), \code{Density} (the mean density of the network),
#' \code{Centralization} (the centralization of the network),
#' \code{Heterogeneity} (the heterogeneity of the network). If the input node
#' significance \code{GS} is specified, the following additional components are
...
#' \code{Connectivity.CF.App, ClusterCoef.CF.App, MAR.CF.App, Density.CF.App,
#' Centralization.CF.App, Heterogeneity.CF.App} giving the conformity-based
...
#' Modules, BMC Systems Biology 2007, 1:24 Horvath S, Dong J (2008) Geometric
...
#' number of rows (and columns) of the adjacency matrix.
#' @return A list with the following components: \item{Connectivity}{a
#' numerical vector that reports the connectivity (also known as degree) of
#' each node. This fundamental network concept is also known as whole network
