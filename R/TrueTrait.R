...
#' datX <- simulateModule(yTRUE, nGenes = 20, minCor = 0.4, maxCor = 0.9,
#'        geneMeans = rnorm(20, 50, 30))
#' True1 <- TrueTrait(datX = datX, y = y)
#' datTrue <- True1$datEstimates
#' \dontrun{
#' # Plot the values and the predicted results
#' par(mfrow = c(2, 2))
...
TrueTrait <- function(datX, y, datXtest = NULL, corFnc = "bicor",
    ...
    if (no.observations > 1) {
        ...
        if (sum(restNonMissingY, na.rm = TRUE) > 3) {
            ...
            weights0 <- rVector^2/((1-rVector^2)*var(y, na.rm = TRUE))
            ...
            if (skipMissingVariables) {
                y.true1 <- as.numeric(apply(as.matrix(datXscaled), 1, mean,
                                            na.rm = TRUE))
                weightsMatrix <- matrix(weights, byrow = TRUE,
                                        nrow = dim(as.matrix(datXscaled))[[1]],
                                        ncol = length(weights))
                weightsMatrix[is.na(datXscaled)] <- 0
                rowsum.weightsMatrix <- apply(as.matrix(weightsMatrix), 1, sum)
                weightsMatrix <- t(scale(t(as.matrix(weightsMatrix)),
                                         center = F,
                                         scale = rowsum.weightsMatrix))
                datXscaledweighted <- as.matrix(datXscaled * weightsMatrix)
                # this corresponds to formula 25 in Klemera et al 2006
                y.true2 <- as.numeric(apply(datXscaledweighted, 1, sum,
                                            na.rm = TRUE))
            }

            # the following is different from Klemera in that it has an absolute
            #  value
            r.characteristic <- sum(rVector^2 / sqrt(1-rVector^2))/sum(
                abs(rVector) / sqrt(1 - rVector^2))
            ...
            SsquaredBE <- var(y.true2 - y, na.rm = TRUE) - (
                1 - r.characteristic^2) / r.characteristic^2 * var(
                    y, na.rm = TRUE)/ no.variables
        ...
        SD.ytrue2 <- sqrt(1 - r.characteristic^2) / r.characteristic * sqrt(
            var(y, na.rm = TRUE) / no.variables)
        # now formula 42
    ...
    if (!is.null(datXtest)) {
        ...
        if (no.variablestest != no.variables) {
            stop("the number of variables in the test data is not the same as",
        ...
        restNonMissingY <- !is.na(y)
        if (sum(restNonMissingY, na.rm = TRUE) > 3) {
            datXtestscaled <- scale(datXtest, center = datCoef[, 1],
                                    scale = datCoef[, 2] )
            y.true1test <- as.numeric(apply(as.matrix(datXtestscaled), 1, mean))
            y.true2test <- as.numeric(as.matrix(datXtestscaled) %*% weights)
            if (skipMissingVariables) {
                y.true1test <- as.numeric(apply(as.matrix(datXtestscaled), 1,
                                                mean, na.rm = TRUE))
                weightsMatrixtest <- matrix(weights,byrow = TRUE,
                                            nrow = dim(as.matrix(datXtestscaled))[[1]],
                                            ncol = length(weights))
                weightsMatrixtest[is.na(datXtestscaled)] <- 0
                rowsum.weightsMatrixtest <- apply(as.matrix(weightsMatrixtest),
                                                  1, sum)
                weightsMatrixtest <- t(scale(t(as.matrix(weightsMatrixtest)),
                                             center = F,
                                             scale = rowsum.weightsMatrixtest))
                datXscaledweightedtest <- as.matrix(
                    datXtestscaled * weightsMatrixtest)
                # this corresponds to formula 25 in Klemera et al 2006
                y.true2test <- as.numeric(apply(datXscaledweightedtest, 1, sum,
                                                na.rm = TRUE))
            } #end of if (skipMissingVariables )
        } # end of if (sum(restNonMissingY,na.rm=TRUE) >3 )

        datEstimatestest <- data.frame(y.true1 = y.true1test,
                                       y.true2 = y.true2test)
    ...
    if ( LeaveOneOut.CV  ) {
        ...
        for ( i in 1:no.observations ) {
            ...
            if (no.observations == 1 ) {
                warning("When dealing with leave one out cross validation, ",
                        "there is only 1 observations in the training data")
            }

            if (no.observations > 1 ) {

                if (addLinearModel) {
                    lmLOO <- lm(y.LOO~., data = data.frame(datX.LOO),
                                na.action = na.exclude)
                    y.lmLOO[i] <-  sum(datXtest.LOO*lmLOO$coeff[-1]) +
                        lmLOO$coeff[[1]]
                }
                ...
                if (skipMissingVariables ) {
                    ...
                    datXscaledweightedLOO <- datXtest.LOOscaled *
                        weightsMatrixLOO
                    y.true2test.LOO[i] <- sum(datXscaledweightedLOO,
                                              na.rm = TRUE)
                } #end of if (skipMissingVariables )
            } # end of for loop
        } # end of if (no.observations>1 )
