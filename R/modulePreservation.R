...
#' to genes in common with with a test set; thus the function calculates a set
...
#' Because the p-values for strongly preserved modules are often extremely low,
...
#' genes or probes. In case of adjacencies, each \code{data} matrix should be a
#' symmetric matrix ith entries between 0 and 1 and unit diagonal.  Each
#' component of the outermost list should be named.
#' @param multiColor a list in which every component is a vector giving the
...
#' calculate co-expression similarity. Defaults to Pearson correlation. Another
#' useful choice is \code{\link{bicor}}. More generally, any function returning
#' values between -1 and 1 can be used.
...
#' @param referenceNetworks a vector giving the indices of expression data to
#' be used as reference networks. Reference networks must have their module
#' labels given in \code{multiColor}.
#' @param testNetworks a list with one component per each entry in
#' \code{referenceNetworks} above, giving the test networks in which to
#' evaluate module preservation for the corresponding reference network. If not
#' given, preservation will be evaluated in all networks (except each reference
#' network). If \code{referenceNetworks} is of length 1, \code{testNetworks}
#' can also be a vector (instead of a list containing the single vector).
#' @param nPermutations specifies the number of permutations that will be
#' calculated in the permutation test.
#' @param includekMEallInSummary logical: should cor.kMEall be included in the
#' calculated summary statistics? Because kMEall takes into account all genes
#' in the network, this statistic measures preservation of the full network
#' with respect to the eigengene of the module. This may be undesirable, hence
#' the default is \code{FALSE}.
#' @param restrictSummaryForGeneralNetworks logical: should the summary
#' statistics for general (not correlation) networks be restricted (density to
#' meanAdj, connectivity to cor.kIM and cor.Adj)? The default \code{TRUE}
#' corresponds to published work.
#' @param calculateQvalue logical: should q-values (local FDR estimates) be
#' calculated? Package qvalue must be installed for this calculation. Note that
#' q-values may not be meaningful when the number of modules is small and/or
#' most modules are preserved.
#' @param randomSeed seed for the random number generator. If \code{NULL}, the
#' seed will not be set. If non-\code{NULL} and the random generator has been
#' initialized prior to the function call, the latter's state is saved and
#' restored upon exit
#' @param maxGoldModuleSize maximum size of the "gold" module, i.e., the random
#' sample of all network genes.
#' @param maxModuleSize maximum module size used for calculations. Modules
#' larger than \code{maxModuleSize} will be reduced by randomly sampling
#' \code{maxModuleSize} genes.
...
#' @param ccTupletSize tuplet size for co-clustering calculations.
#' @param calculateCor.kIMall logical: should cor.kMEall be calculated? This
#' option is only valid for adjacency input. If \code{FALSE}, cor.kIMall will
#' not be calculated, potentially saving significant amount of time if the
#' input adjacencies are large and contain many modules.
...
#' are labeled by grey color or numeric label 0. These values are the default
#' when \code{multiColor} contains character or numeric vectors, respectively.
...
#' @param interpolationPlotFile file name to save the interpolation plots into.
#' @param discardInvalidOutput logical: should output columns containing no
#' valid data be discarded? This option may be useful when input
#' \code{dataIsExpr} is \code{FALSE} and some of the output statistics cannot
#' be calculated. This option causes such statistics to be dropped from output.
#' @param parallelCalculation logical: should calculations be done in parallel?
#' Note that parallel calculations are turned off by default and will lead to
#' somewhat DIFFERENT results than serial calculations because the random seed
#' is set differently. For the calculation to actually run in parallel mode, a
#' call to \code{\link{enableWGCNAThreads}} must be made before this function
#' is called.
...
#' @return The function returns a nested list of preservation statistics. At
#' the top level, the list components are: \item{quality}{observed values, Z
#' scores, log p-values, Bonferoni-corrected log p-values, and (optionally)
#' q-values of quality statistics. All logarithms are in base 10.}
#' \item{preservation }{observed values, Z scores, log p-values,
#' Bonferoni-corrected log p-values, and (optionally) q-values of density and
#' connectivity preservation statistics. All logarithms are in base 10.}
#' \item{accuracy}{observed values, Z scores, log p-values, Bonferoni-corrected
#' log p-values, and (optionally) q-values of cross-tabulation statistics. All
#' logarithms are in base 10.} \item{referenceSeparability}{observed values, Z
#' scores, log p-values, Bonferoni-corrected log p-values, and (optionally)
#' q-values of module separability in the reference network. All logarithms are
#' in base 10.} \item{testSeparability}{observed values, Z scores, p-values,
#' Bonferoni-corrected p-values, and (optionally) q-values of module
#' separability in the test network. All logarithms are in base 10.}
#' \item{permutationDetails}{results of individual permutations, useful for
#' diagnostics} All of the above are lists. The lists \code{quality},
#' \code{preservation}, \code{referenceSeparability}, and
#' \code{testSeparability} each contain 4 or 5 components: \code{observed}
#' contains observed values, \code{Z} contains the corresponding Z scores,
#' \code{log.p} contains base 10 logarithms of the p-values, \code{log.pBonf}
#' contains base 10 logarithms of the Bonferoni corrected p-values, and
#' optionally \code{q} contains the associated q-values. The list
#' \code{accuracy} contains \code{observed}, \code{Z}, \code{log.p},
...
#' components correspond to reference sets and the inner components to tests
#' sets. As an example, \code{preservation$observed[[1]][[2]]} contains the
#' density and connectivity preservation statistics for the preservation of set
#' 1 modules in set 2, that is set 1 is the reference set and set 2 is the test
#' set. \code{preservation$observed[[1]][[2]]} is a data frame in which each
#' row corresponds to a module in the reference network 1 plus one row for the
