## This file contains several functions which can be used to adjust the
##  dendrogram in ways which keep the dendrogram mathematically identical (ie,
##  branch swapping, branch reflection, etc).
...
#' dat1 <- simulateDatExpr(ME, 400, c(0.16, 0.12, 0.11, 0.10, 0.10, 0.10, 0.1),
#'                         signed = TRUE)
...
#' }
#' ## Reassign modules using the selectBranch and chooseOneHubInEachModule
#' ##functions
...
#'                     dendroLabels = FALSE)
#' }
#' ## Now swap and reflect some branches, then optimize the order of the
#' ## branches and output pdf with resulting images
#' \dontrun{
...
orderBranchesUsingHubGenes <- function(hierTOM, datExpr = NULL, colorh = NULL,
	...
	if (is.null(adj)) {
		...
		adj <- adjacency(datExpr[, genes], type = type,
		                 power = (2-(type == "unsigned")))
		colnames(adj) <- rownames(adj) <- genes
	}
	genes <- rownames(adj)
	if (length(genes) != length(intersect(genes, hGenes))) {
		stop("All genes in the adjacency must also be in the gene tree. ",
		     " Check to make sure that names(hierTOM$labels) is set to the ",
		     "proper gene or probe names and that these correspond to the ",
		     "expression / adjacency gene names.")
	}
	...
	iters <- (1:iter)/iter
	# Quickly decreasing chance of random swap
	swapAnyway <- rep(0, length(iters))
	if (allowNonoptimalSwaps) {
	    swapAnyway <- ((1-iters)^3)/3+0.001
	}
	...
	for (i in 1:iter) {
		...
		if ((newSum > oldSum) | ((sample(1:1000, 1)/1000) < swapAnyway[i])) {
			hierTOM <- hierTOMnew
			changes <- rbind(changes, c(i, ifelse(swap == 1, "Swap","Reflect"),
			                            g, oldSum, newSum))
			adj <- adjNew
		}
		message("Interation ", i, " of ", iter)
		collectGarbage()
	}
...
# Output the results
	colnames(changes) <- c("Iter.#", "Swap?", "Gene1", "Gene2", "OldScore",
	                       "NewScore")
...
selectBranch <- function (hierTOM, g1, g2) {
    ...
    cond <- (length(g1) == 0) | (length(g2) == 0) | (
        max(c(g1,g2)) > length(hierTOM$labels))
	if (cond) {
		warning("Input genes are not both legal indices")
		return(hierTOM)
	}
...
reflectBranch <- function (hierTOM, g1, g2, both = FALSE) {
	...
	if (both) {
	    b1 <- c(b1,selectBranch(hierTOM, g2, g1))
	}
...
#' ME <- data.frame(MEturquoise, MEblue, MEbrown, MEyellow, MEgreen, MEred,
#'                  MEblack)
#' dat1 <- simulateDatExpr(ME, 300,
#'                         c(0.2, 0.1, 0.08, 0.051, 0.05, 0.042, 0.041, 0.3),
#'                         signed = TRUE)
#' TOM1 <- TOMsimilarityFromExpr(dat1$datExpr, networkType = "signed")
#' colnames(TOM1) <- rownames(TOM1) <- colnames(dat1$datExpr)
...
chooseOneHubInEachModule <- function(datExpr, colorh, numGenes = 100,
	...
	out <- chooseTopHubInEachModule(datExpr, colorh, omitColors, power, type,
	                                ...)
	return(out)
}
...
chooseTopHubInEachModule <- function(datExpr, colorh, omitColors = "grey",
	...
	if (is.null(colnames(datExpr))) {
		colnames(datExpr) <- 1:ncol(datExpr)
		isIndex <- TRUE
	}
