...
#' @description
#' The function uses a rather primitive way to estimate available memory and use
#' it to suggest a block size appropriate for the many block-by-block
...
#' rectangular (of size blockSize times matrixSize) or square (of size blockSize
#' times blockSize).
#' @param maxMemoryAllocation maximum desired memory allocation, in bytes.
#' Should not exceed 2GB or total installed RAM (whichever is greater) on 32-bit
#' systems, while on 64-bit systems it should not exceed the total installed RAM.
#' If not supplied, the available memory will be estimated internally.
...
#' known as block-by-block, or block-wise) approach to handling large data sets.
#' This function is meant to assist in choosing a suitable block size, given the
#' size of the data and the available memory.
#'
#' If the entire expected result fits into the allowed memory (after taking into
#' account the expected overhead), the returned block size will equal the input
#' matrixSize.
#'
...
#' @return
#' A single integer giving the suggested block size, or matrixSize if the entire
#' calculation is expected to fit into memory in one piece.
#' @author
#' Peter Langfelder
#' @examples
