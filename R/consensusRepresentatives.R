.checkConsistencyOfGroupAndColID <- function(mdx, colID, group) {
  ...
  if ( sum(is.na(colID))>0 )
      warning("The argument colID contains missing data. It is recommend you ",
              "choose non-missing,\nunique values for colID, e.g. character ",
              "strings.")

   if ( sum(group=="",na.rm=TRUE)>0 ){
      warning("group contains blanks. It is strongly recommended that you ",
              "remove these rows before calling the function.\n   But for your",
              " convenience, the collapseRow function will remove these rows")
      group[group == ""]=NA
   }

   if ( sum(is.na(group))>0 ){
       warning("The argument group contains missing data. It is strongly ",
               "recommended\n   that you remove these rows before calling the ",
               "function. Or redefine group\n   so that it has no missing ",
               "data. But for convenience, we remove these data.")
   }
...
#   The main part of this function is run only if omitGroups=TRUE
   ...
   if (sum(apply(mdx, function(x) sum(is.na(x)))) == 0)
...
#   Otherwise, use "method" if there are 3+ ps/g as well.
   if(!is.null(connectivityPower)){
     ...
     if(any(nSamples<=5)){
       warning("5 or fewer samples, this method of probe collapse is ",
               "unreliable......Running anyway, but we suggest trying another ",
               "method (for example, *mean*).")
     }
   }
   ...
   selectionStatistics = apply(mdx, function(x)
 ...
 #  }
   ...
   if (length(more) > 0)
   {
     ...
     for (ig in 1:length(more)) {
        ...
        mdxTmp = subset(mdx, , keepProbes1)
        adj = apply(mdxTmp, function(x) do.call(adjacency,
                c(list(x, type = "signed", power = connectivityPower), adjacencyArguments)))
        connectivities[keep.inMore, ] = apply(adj, colSums)
        count = count + 1
   ...
   reprIndicator[keepVars] [match(representatives, colnames(mdx))] = TRUE
