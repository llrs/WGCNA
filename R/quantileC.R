...
colQuantileC <- function(data, p) {
  ...
  if (length(p) > 1) {
    stop("This function only calculates one quantile at a time")
  }
  if ( (p<0) || (p>1)) {
    stop("Probability ", p, " is out of the allowed range between 0 and 1.")
  }
  res <- .C("quantileC", data = as.double(data), nrow = as.integer(nrow),
           ncol = as.integer(ncol), p = as.double(p),
           quantiles = as.double(quantiles), NAOK = TRUE)
...
rowQuantileC = function(data, p) {
  ...
  res <- .C("rowQuantileC", data = as.double(data), nrow = as.integer(nrow),
           ncol = as.integer(ncol), p = as.double(p),
           quantiles = as.double(quantiles), NAOK = TRUE)
